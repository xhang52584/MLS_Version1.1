...
def compute_freqs_kernel(
    ...
    *** TODO: Implement this kernel ***
    ...
    # Step 5: Store concatenated cos/sin
    curr_cos_row_ptr = cos_ptr + pid * stride_cos0
    curr_sin_row_ptr = sin_ptr + pid * stride_sin0

    tl.store(curr_cos_row_ptr + offs * stride_cos1, cos_half, mask=mask)
    tl.store(curr_sin_row_ptr + offs * stride_sin1, sin_half, mask=mask)

    tl.store(curr_cos_row_ptr + (offs + half_dim) * stride_cos1, cos_half, mask=mask)
    tl.store(curr_sin_row_ptr + (offs + half_dim) * stride_sin1, sin_half, mask=mask)
